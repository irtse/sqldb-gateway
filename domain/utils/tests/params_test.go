...
func TestParams_Enrich(t *testing.T) {
	params := utils.Params{"key": "old"}
	enriched := params.Enrich(map[string]interface{}{"key": "new", "extra": "data"})
	assert.Equal(t, "new", enriched["key"])
	assert.Equal(t, "data", enriched["extra"])
}

func TestParams_Delete(t *testing.T) {
	params := utils.Params{"key": "value", "remove": "this"}
